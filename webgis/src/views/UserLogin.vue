...
import { login, checkSession } from '@/api/user'
...
const validateInput = () => {
  isFormValid.value = loginForm.username.length > 0 && loginForm.password.length > 0
...
const handleLogin = async () => {
  try {
    ...
    if (response.data.status === 'success') {
      ...
      errorMsg.value = response.data.message || '登录失败，请稍后重试'
    }
  } catch (error) {
    if (error.response) {
